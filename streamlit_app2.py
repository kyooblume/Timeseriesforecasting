...
st.write("このコードは実行されていますか？")  # 条件分岐の外に書いて試す
...
if phase == "1.データのアップロード":
    ...
    if df is not None:
        ...
        if "yyyymmdd" in df.columns:
            ...
            # --- ② 予測結果をデータフレームに追加（元データを壊さず） ---
            train_pred_df = X_train.copy()
            train_pred_df["predicted"] = train_pred  # ← 別名にするのが重要

            test_pred_df = X_test.copy()
            test_pred_df["predicted"] = test_pred

            # --- ③ ターゲット変数（正解ラベル）は y_train / y_test をそのまま使う ---
            X_train_pred = train_pred_df[selected_features]  # 特徴量のみ
            y_train_pred = y_train                           # ← 正解ラベル！

            X_test_pred = test_pred_df[selected_features]
            y_test_pred = y_test

            # --- ④ 再学習 or 次ステップで利用 ---
            model.fit(X_train_pred, y_train_pred)
            y_train_pred_update = model.predict(X_train_pred)
            ...
            fig, ax = plt.subplots(figsize=(12, 6))
            ax.plot(future_dates[:len(forecast)], forecast, label="Forecast", marker="x")
            plt.xticks(rotation=45)
            plt.legend()
            st.pyplot(fig)
